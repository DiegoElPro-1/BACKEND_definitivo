...
export default class extends BaseSchema {
  ...
  async up() {
    this.schema.createTable(this.tableName, (table) => {
      ...
      table.integer('id_punto').unsigned().notNullable().references('id_punto').inTable('puntos_reciclaje').onDelete('RESTRICT')
      table.integer('id_estado_entrega').unsigned().notNullable().references('id_estado_entrega').inTable('estados_entregas').onDelete('RESTRICT')
