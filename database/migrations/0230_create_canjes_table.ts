...
export default class extends BaseSchema {
  ...
  async up() {
    this.schema.createTable(this.tableName, (table) => {
      ...
      table.integer('id_recompensa').unsigned().notNullable().references('id_recompensa').inTable('recompensas').onDelete('RESTRICT')
      table.integer('id_estado_canje').unsigned().notNullable().references('id_estado_canje').inTable('estados_canjes').onDelete('RESTRICT')
